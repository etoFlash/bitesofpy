...
def _get_characters(characters_list):
    for character in characters_list:
        if type(character) == str:
            yield character.lower(), character.lower()
        else:
            alias = character[0]
            for s in character:
                yield alias.lower(), s.lower()
...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    for i, line in enumerate(text.lower().splitlines()):
        for a, c in _get_characters(characters):
            if c in line and i not in data[a]:
                data[a].append(i)
